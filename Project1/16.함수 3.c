...
// 3. 가변인자의 경우는 전달받은 값을 알 수가 없습니다. 그래서 처음 받는 값은 보통 받은 값의 개수를 
//    알 수 있는 데이터를 전달합니다.
...
// 7. va_end(args) : 가변인자에 대한 처리를 종효하고 기능을 제거합니다. (제거 안하면 프로그램에 데이터 남음 필수로 제거)
...
// 2. 배열을 받는 경우
// 배열을 전달할 경우에는 c언어가 자동적으로 배열의 길이를 확인할 수 없기 때문에
// 따로 배열의 길이는 작성해줘야합니다.
