...
// 3. 재귀 호출 : 조건을 만족했을 경우 다음 단계로 진해합니다.
...
{
	// 4. 조건) 현재까지 선택된 수가 M과 동일 한 경우의 작업
	if (depth == M)
	{
		// 설정된 결과 값 출력
		for (int i = 0; i < M; i++)
	...
	for (int i = 1; i <= N; i++)
	{
		// 6. path[i]가 사용되지 않는 상태라면(used[i]의 값이 0인 경우)
		if (!used[i])
		{
			used[i] = 1; // 사용 중이라고 설정하겠습니다. (c 프로그래밍에서 1은 ture, 0은 false를 의미합니다.
			...
			// 8. 위의 작업이 마무리 되면 사용했던 used[i]를 다시 0으로 만들어서 비사용 처리 진행
			used[i] = 0;
		}
	}
...
{
	int N, M;
	scanf_s("%d %d\n", &N, &M); // N과 M을 순서대로 입력(띄어쓰기나 ENTER 기준)

	Backtracking(N, M, 0, path);
	// 배열의 시작 값이 0이기 때문에 depth는 0을 넣어서 순차적을 작업이 진행될 것입니다.

	// 해당 문제의 작업 순서
	// 1. scanf에 의해 N과 M이 결정됩니다. ex) 3 2
	// 2. Backtracking(3,2,0,path) 호출이 진해됩니다.
