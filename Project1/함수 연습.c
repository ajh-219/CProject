...
// 중안 값은 주어진 수를 크기 순서대로 정리했을 경우, 가장 중앙에 있는 값을 의미
...
int center(int *numbers, int size)
{
	...
	for (int i = 0; i < size; i++)
	{
		// 전체 사이즈보다 1작은 범위부터 시작(처음 값과 다음 값을 비교하기 때문)
...
int main()
{
	...
	int avg_value = avg(sum_value, 5); // 배열의 평균 = avg(배열, 배열개수);
	int center_value = center(numbers, 5); // 배열의 중앙값 = (배열, 배열개수);
