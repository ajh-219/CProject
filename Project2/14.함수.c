...
// 3. 사용자는 c언어에서 제고해주는 함수 이외에 사용자 정의 함수를 정의, 구현할 수 있습니다.
...
// 4. return 코드를 실행하면, 함수가 종료됩니다. 추가적으로 return 다음에 갑싱 작성되어있는 경우 함수를 호출한 위치에 그 값을 전달합니다.
...
int sub(int A, int B)
{
	...
	return result;
	// return a - b; 간달한 계산 결과라면 return을 통해 한번에 작성하는게 위의 방식보다 읽기가 편합니다.
}
